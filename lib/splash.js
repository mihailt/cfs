...
(function() {
  ...
  Keyboard = (function() {
    ...
    function Keyboard() {
      ...
      for (k in _ref) {
        v = _ref[k];
        this.down[k] = this.down[v] = this.pressed[k] = this.pressed[v] = this.released[k] = this.released[v] = false;
      }
      _ref1 = ['keyup', 'keydown'];
    ...
    Keyboard.prototype.handler = function(event) {
      ...
      this.down[code] = this.down[key] = event.type === 'keydown';
  ...
  Mouse = (function() {
    ...
    function Mouse(element) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        b = _ref[_i];
        this.pressed[b] = this.down[b] = this.released[b] = this.clicked[b] = false;
      }
      this.map = {
        0: 'left',
        1: 'middle',
      ...
      _ref1 = ['click', 'mousemove', 'mousedown', 'mouseup', 'contextmenu'];
    ...
    Mouse.prototype.handler = function(event) {
      ...
      this.px = tmpx || this.x;
      this.py = tmpy || this.y;
      if ((_ref = event.type) === 'mousedown' || _ref === 'mouseup' || _ref === 'click') {
        code = event.button;
        key = this.name(code);
        this.pressed[code] = this.pressed[key] = event.type === 'mousedown';
        this.down[code] = this.down[key] = event.type === 'mousedown';
        this.released[code] = this.released[key] = event.type === 'mouseup';
        return this.clicked[code] = this.clicked[key] = event.type === 'click';
      }
    };
